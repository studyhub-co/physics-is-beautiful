...
class LessonsSearchView extends React.Component {
  ...
  componentWillReceiveProps(props) {
    if (
      ...
      props.selectedTab === 'Lessons'
    ) {
      this.props.loadSearchLessons(this.props.searchString)
    }

    if (
      this.props.lessonsSearchList !== props.lessonsSearchList &&
      props.lessonsSearchList
    ) {
      if (props.lessonsSearchList.previous == null) {
        ...
        var newlist = [
          ...this.state.lessons,
          ...props.lessonsSearchList.results,
        ]
  ...
  render() {
    ...
    this.state.lessons.map((lesson, i) => {
      items.push(<LessonThumbnailPublic key={lesson.uuid} lesson={lesson} />)
    })

    return (
      <Container fluid>
        {this.props.lessonsSearchList ? (
          <div>
            <InfiniteScroll
              pageStart={0}
            ...
            {this.props.lessonsSearchList.results.length === 0 ? (
...
const mapDispatchToProps = dispatch => {
  return {
    dispatch,
    loadSearchLessons: (searchString, url) =>
      dispatch(loadSearchLessons(searchString, url)),

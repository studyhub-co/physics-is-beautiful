...
class BrowseCoursesView extends React.Component {
  ...
  onAddRemoveFromDashboardSildes (action, course) {
    ...
    if (action === 'add') {
      if (!this.alreadyInSlides(newRecent, course.uuid)) {
        newRecent.push(<CourseThumbnailPublic
  ...
  componentWillReceiveProps (props) {
    for (var i = 0, len = slidesNames.length; i < len; i++) {
      ...
      if (props[prefix + 'Courses'] && props[prefix + 'Courses'] !== this.props[prefix + 'Courses']) {
            ...
            >
              ...
              <div className='content'>
                <TabContent for='Courses'>
                  { this.state.searchEnabeled
                    ? <CoursesSearchView
                      ...
                      coursesSearchString={this.state.searchString} /> : null
                  }
                ...
                <TabContent for='Materials'>
                  <MaterialsSearchView
                    ref={(node) => { if (node) this.searchMaterialsView = node }}
                    searchString={this.state.searchString}
                    selectedTab={this.state.selectedTab}
                  />
                </TabContent>
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    loadPopularCourses: (url) => dispatch(loadAllCourses(url, null, '-number_of_learners_denormalized')) // popular
  }
}

...
class CourseThumbnailPublic extends React.Component {
  constructor(props, context) {
    ...
    this.onRemoveFromDashboardSelect = this.onRemoveFromDashboardSelect.bind(
      this,
    )
  ...
  onAddToDashboardSelect(e) {
    ...
    if (this.props.slidesListName !== 'recentSlides') {
      this.props.onAddRemoveFromDashboardSlides('add', this.props.course)
    }
  }
  ...
  onRemoveFromDashboardSelect(e) {
    ...
    if (this.props.slidesListName === 'recentSlides') {
      this.props.onAddRemoveFromDashboardSlides('remove', this.props.course)
    }
  }
  ...
  render() {
      ...
      ReactDOM.createPortal(
        <Dropdown.Menu rootCloseEvent={'mousedown'}>
          ...
          {this.props.slidesListName === 'recentSlides' ? (
            <Dropdown.Item
              onSelect={this.onRemoveFromDashboardSelect}
              eventKey="5"
            >
    ...
    return (
      <div className={'course-card'} style={{ cursor: 'pointer' }}>
        ...
        <div>
          <Dropdown style={{ float: 'right' }} id="dropdown-custom-menu">
          ...
          >
            <a
              href={history.createHref({
                pathname: this.props.course.author.get_absolute_url,
              })}
            >
              {this.props.course.author.display_name}
            </a>{' '}
            ∙ {this.props.course.count_lessons} lessons ∙{' '}
            {this.props.course.number_of_learners} learners
          </div>
          <div
            style={{
              fontSize: '1rem',
          ...
          >
            Created <Moment fromNow>{this.props.course.created_on}</Moment> ∙
            Last updated <Moment fromNow>{this.props.course.updated_on}</Moment>
          </div>
        </div>
      </div>
    )
...
const mapDispatchToProps = dispatch => {
  return {
    ...
    removeCourseFromDashboard: uuid =>
      dispatch(removeCourseFromDashboard(uuid)),
  }
}

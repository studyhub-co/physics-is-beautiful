...
class ThumbnailMenu extends React.Component {
  ...
  componentDidMount() {
    if (
      Object.entries(this.props.myCourses).length === 0 &&
      !this.props.myCourses.hasOwnProperty('isFetching')
    ) {
  ...
  onLearnSelect() {
    if (this.state.baseName === 'material') {
      // we can open material directly now
      history.push(
        `/courses/lessons/${
          this.props[this.state.baseName].lesson.uuid
        }/materials/${this.props[this.state.baseName].uuid}/`,
        '_self',
      )
    } else {
      history.push(
        '/courses/' +
          this.state.baseName +
          's/' +
          this.props[this.state.baseName].uuid +
          '/',
        '_self',
      )
    }
  ...
  addElementToNewCourse() {
    this.props.addToNewCourse(
      this.state.baseName,
      this.props[this.state.baseName],
    )
  }
  ...
  render() {
    ...
    if (this.state.level === 1) {
      ...
      if (this.state.baseName === 'material') {
        learnText = 'Learn material'
        copyText = 'Copy link to material'
      }
      menus.push(
        <Dropdown.Item onSelect={this.onLearnSelect} key="1" eventKey="1">
          <FaGraduationCap />
          &nbsp;{learnText}
        </Dropdown.Item>,
      )
      menus.push(
        <Dropdown.Item onSelect={this.onForkSelect} key="3" eventKey="3">
          <FaCodeBranch />
          &nbsp;Fork to course studio >
        </Dropdown.Item>,
      )
      menus.push(
        <Dropdown.Item onSelect={this.onCopyShareableLink} key="4" eventKey="4">
          <FaShareAlt />
          &nbsp;{copyText}
        </Dropdown.Item>,
      )
    ...
    if (this.state.level === 2) {
      menus.push(
        <Dropdown.Item onSelect={this.onBack} key={'21'}>
          {'< Back'}
        </Dropdown.Item>,
      )
      ...
      menus.push(
        <Dropdown.Item
          onSelect={this.addElementToNewCourse}
          key="4"
          eventKey="4"
          style={{ color: 'blue' }}
        >
    ...
    if (this.state.level === 5) {
      ...
      var lessonsUuidsList = this.props.modules[this.state.selectedModule.uuid]
        .lessons
...
const mapDispatchToProps = dispatch => {
  return {
    ...
    addMaterial: (lessonUuid, material) =>
      dispatch(addMaterial(lessonUuid, material)),
    loadModuleIfNeeded: moduleUuid => dispatch(loadModuleIfNeeded(moduleUuid)),
    loadMyCourses: () => dispatch(loadCourses()),

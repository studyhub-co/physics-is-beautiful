...
class UnitComponent extends React.Component {
    ...
    if (
      confirm(
        'This will permanently delete unit "' +
          this.props.name +
          '" with all its contents. Are you sure?',
      )
    ) {
  ...
  render() {
    ...
    for (var i = 0; i < this.props.modules.length; i++) {
      modules.push(
        <DockableDropTarget
          key={this.props.modules[i]}
          onDrop={this.props.onModuleDroppedBefore.bind(
            null,
            this.props.modules[i],
          )}
          itemType={DragItemTypes.MODULE}
          selfUuid={this.props.modules[i]}
        >

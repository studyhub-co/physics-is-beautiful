...
class IsAssignmentClassroomTeacherOrStudentReadonly(permissions.BasePermission):

    def has_permission(self, request, view):
        return True

    def _is_teacher_or_student(self, user, obj, safe):
        if user == obj.classroom.teacher.user or (obj.classroom.students.filter(user__pk=user.pk).count() > 0 or safe):
            return True
        else:
            return False

...
import {
  ...
  ASSIGNMENT_RECEIVE_ASSIGNMENT_SUCCESS,
  ASSIGNMENT_UPDATE_ASSIGNMENT_SUCCESS,
  ASSIGNMENTS_RECEIVE_ASSIGNMENT_STUDENTS_LIST,
...
export function assignmentFetchAssignment(
  classroomUuid,
  assignmentUuid,
  callback = null,
) {
  return (dispatch, state) => {
    return getAxios()
      .get(API_PREFIX + classroomUuid + '/assignment/' + assignmentUuid + '/')
...
export function assignmentFetchStudentsList(
  classroomUuid,
  assignmentUuid,
  callback = null,
) {
...
export function assignmentFetchAssignmentList(classroomUuid) {
  return (dispatch, state) => {
    return getAxios()
      .get(API_PREFIX + classroomUuid + '/assignment/')
      .then(checkHttpStatus)
      .then(response => {
...
export function assignmentCreateAssignment(
  assignmentJson,
  refreshAssignmentsList = false,
) {
  return (dispatch, state) => {
    return getAxios()
      .post(
        API_PREFIX + assignmentJson.classroom_uuid + '/assignment/',
        assignmentJson,
      )
      .then(checkHttpStatus)
      .then(response => {
...
export function assignmentPartialUpdateAssignment(
  assignmentJson,
  refreshAssignmentsList = false,
  callback = null,
) {
...
export function assignmentDeleteAssignment(
  classroomUuid,
  assignmentUuid,
  refreshAssignmentsList = false,
  callback = null,
  ...
  return (dispatch, state) => {
    return getAxios()
      .delete(
        API_PREFIX + classroomUuid + '/assignment/' + assignmentUuid + '/',
      )
      .then(checkHttpStatus)
      .then(response => {
...
export function assignmentFetchFirstUncompletedLesson(
  classroomUuid,
  assignmentUuid,
  callback = null,
) {
...
export function receiveAssignmentStudentLessonsList(
...
export function assignmentFetchStudentLessonsList(
  classroomUuid,
  assignmentUuid,
  callback = null,
) {
  return (dispatch, state) => {
    return getAxios()
      ...
      .then(response => {
        dispatch(
          receiveAssignmentStudentLessonsList(response.data, assignmentUuid),
        )
        if (typeof callback === 'function') {
          callback()
        }

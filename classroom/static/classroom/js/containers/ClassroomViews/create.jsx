...
class CreateClassroomView extends React.Component {
  ...
  // componentWillMount() {
  componentDidMount() {
    const { history, match, classroomTeacher } = this.props
    ...
    if (match.params && match.params['uuid']) {
      ...
      if (!classroomTeacher) {
        ...
        this.setState(
          {
            classroomFormValues: {
              name: classroomTeacher.name,
              course_uuid: classroomTeacher.course.uuid,
            },
          },
  ...
  handleClassroomFormChange(e) {
    ...
    this.setState({ classroomFormValues }, this.validateClassroomForm)
  }
  ...
  validateClassroomForm() {
    ...
    if (
      this.state.classroomFormValues &&
  ...
  saveClassroom(e) {
    ...
    if (this.state.classroomFormIsValid) {
      if (!this.props.classroomTeacher) {
        // save
        this.props.classroomActions.classroomCreateClassroom(
          this.state.classroomFormValues,
          true,
          null,
          true,
        ...
        this.props.classroomActions.classroomPartialUpdateTeacherClassroom(
          Object.assign({}, this.state.classroomFormValues, {
      ...
      <PopupWindow goBack={history.goBack}>
        <form onSubmit={this.saveClassroom}>
          ...
          <button
            ...
            className={
              'classroom-common-button float-right' +
              (this.state.classroomFormIsValid ? '' : ' disabled-button')
            }
            type="submit"
          >
            {this.props.classroomTeacher
...
export default withRouter(
  connect(mapStateToProps, mapDispatchToProps)(CreateClassroomView),
)
